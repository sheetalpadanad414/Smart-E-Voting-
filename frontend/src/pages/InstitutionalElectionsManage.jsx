...
const InstitutionalElectionsManage = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    start_date: '',
    end_date: '',
    election_type: 'college',
    scope: 'institutional'
  });

  const INSTITUTIONAL_TYPES = ['college', 'university', 'company', 'society'];
  ...
  const fetchElections = async () => {
    try {
      ...
      const institutionalElections = response.data.elections.filter(
        e => e.scope === 'institutional' || INSTITUTIONAL_TYPES.includes(e.election_type?.toLowerCase())
      );
      setElections(institutionalElections);
    } catch (error) {
  ...
  const handleEdit = (election) => {
    ...
    setFormData({
      ...
      election_type: election.election_type?.toLowerCase() || 'college',
      scope: 'institutional'
    });
    setShowForm(true);
  };
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 p-6">
      <div className="max-w-7xl mx-auto">
        <div className="flex justify-between items-center mb-6">
          <div>
            <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Institutional Elections</h1>
            <p className="text-gray-600 dark:text-gray-400 mt-1">Manage College, University, Company, and Society elections</p>
          </div>
          <button
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 dark:bg-gray-700 dark:text-white"
                    placeholder="e.g., XYZ College Student Council Election 2024"
                    required
                  />
                </div>
              </div>
            ...
            <table className="w-full">
              ...
              <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                {loading ? (
                  ...
                  <tr>
                    <td colSpan="6" className="px-6 py-8 text-center text-gray-500 dark:text-gray-400">
                      No institutional elections found. Create one to get started.
                    </td>
                  </tr>
                ) : (
                  elections.map((election) => (
                    <tr key={election.id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                      ...
                      <td className="px-6 py-4 text-sm text-gray-600 dark:text-gray-400">{getTypeLabel(election.election_type)}</td>
...
export default InstitutionalElectionsManage;
